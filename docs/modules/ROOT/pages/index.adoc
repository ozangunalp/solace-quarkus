...
NOTE: Solace Quarkus Extension 3.1.1 is based on Quarkus 3.15.x with Java 17 as baseline.

Starting from 3.1.1 the extension also supports Solace JCSMP API alongside Solace Java Messaging API. This extension can be used in two ways

{empty}1. `com.solace.quarkus:quarkus-solace-jcsmp-client`

This extension provides only Solace JCSMP Messaging API and users need to have their own implementation and configuration to interact with Solace PubSub+ broker.

If you want to use this extension, you need to add the `com.solace.quarkus:quarkus-solace-jcsmp-client` extension first to your build file.
...
<dependency>
    <groupId>com.solace.quarkus</groupId>
    <artifactId>quarkus-solace-jcsmp-client</artifactId>
    <version>3.1.1</version>
</dependency>
----

{empty}2. `com.solace.quarkus:quarkus-solace-jcsmp-messaging-connector`

This extension is based on reactive messaging framework and provides pre-defined configurations for incoming and outgoing channels.

If you want to use this extension, you need to add the `com.solace.quarkus:quarkus-solace-jcsmp-messaging-connector` extension first to your build file.
...
<dependency>
    <groupId>com.solace.quarkus</groupId>
    <artifactId>quarkus-solace-jcsmp-messaging-connector</artifactId>
    <version>3.1.1</version>
</dependency>
----

NOTE: All configurations are supported, except for back-pressure configuration.
...
----
Solace JCSMP API Configuration

[source,yaml]
----
quarkus.solace.host=tcp://localhost:55555
quarkus.solace.vpn=default
quarkus.solace.username=test
quarkus.solace.password=test
----
...
quarkus.solace.username=test
quarkus.solace.password=test
quarkus.solace.SSL_TRUST_STORE=
quarkus.solace.SSL_TRUST_STORE_FORMAT=
quarkus.solace.SSL_TRUST_STORE_PASSWORD=
----
...
quarkus.solace.host=tcps://localhost:55443
quarkus.solace.vpn=default
quarkus.solace.AUTHENTICATION_SCHEME=AUTHENTICATION_SCHEME_CLIENT_CERTIFICATE
quarkus.solace.SSL_TRUST_STORE=
quarkus.solace.SSL_TRUST_STORE_FORMAT=
quarkus.solace.SSL_TRUST_STORE_PASSWORD=
quarkus.solace.SSL_KEY_STORE=
quarkus.solace.SSL_KEY_STORE_PASSWORD=
quarkus.solace.SSL_KEY_STORE_FORMAT=
----
...
quarkus.solace.host=tcp://localhost:55555
quarkus.solace.vpn=default
quarkus.solace.AUTHENTICATION_SCHEME=AUTHENTICATION_SCHEME_OAUTH2
quarkus.solace.oidc.client-name=solace // client name provided in oidc client config below
quarkus.solace.oidc.refresh.interval=50s // Refresh interval should be less than access token expiry time. Otherwise extension will fail to update access token in solace session.
quarkus.solace.oidc.refresh.timeout=10s // Token Refresh API timeout. Default is set to 10 seconds.
...
quarkus.solace.host=tcp://localhost:55555
quarkus.solace.vpn=default
quarkus.solace.username=basic
quarkus.solace.password=basic

mp.messaging.incoming.temperatures.connector=quarkus-solace-jcsmp
mp.messaging.incoming.temperatures.consumer.queue.name=temperatures
----
...
public class TemperaturesConsumer {
    ...
    public CompletionStage<Void> consume(Message<byte[]> msg) {
        ...
        double temperature = Double.parseDouble(new String(msg.getPayload()));
...
__InboundMessage__ This is the incoming Inbound Message from Solace Java Messaging API
...
__BytesXMLMessage__ This is the incoming Inbound Message from Solace Java Messaging API
...
public class TemperaturesConsumer {
    @Incoming("temperatures")
    public void consume(BytesXMLMessage inboundMessage) {
        // process the message payload.
        String temperature = SolaceMessageUtils.getPayloadAsString(inboundMessage);
    }
}
----
...
mp.messaging.outgoing.temperatures-out.connector=quarkus-solace-jcsmp
mp.messaging.outgoing.temperatures-out.producer.topic=temperatures
----
...
NOTE: This feature is not supported in Quarkus Solace JCSMP Messaging Connector
...
public class TemperaturesProcessor {
    ...
    public double process(byte[] temperature) {
        return (Double.parseDouble(new String(temperature)) - 32) * 5 / 9;
    }
